...
>
> El seguimiento del trabajo se hizo en GitHub Issues: épicas [#20](https://github.com/luisurcia/mini_erp/issues/20) · [#36](https://github.com/luisurcia/mini_erp/issues/36) · [#47](https://github.com/luisurcia/mini_erp/issues/47) · [#76](https://github.com/luisurcia/mini_erp/issues/76) (personalización base + rondas de ajustes de UX), [#87](https://github.com/luisurcia/mini_erp/issues/87) (quinta ronda: PDF de cobranza, métricas de Dashboard, **flujo de bodegas Fermentación → Principal → distribución**), [#90](https://github.com/luisurcia/mini_erp/issues/90) (feedback: PDF agrupado por cliente, **consumo de insumos al armar**), [#106](https://github.com/luisurcia/mini_erp/issues/106) (feedback: motivo "armado" traducido, **filtro de cliente en la lista de Ventas**), [#95](https://github.com/luisurcia/mini_erp/issues/95) (sexta ronda: **módulo de Compras**, columnas del grid de venta ordenadas por lo más vendido), [#112](https://github.com/luisurcia/mini_erp/issues/112) (séptima ronda: **categoría de Compras como selector + mantenedor**, **ticket de despacho**), [#115](https://github.com/luisurcia/mini_erp/issues/115) (feedback del ticket de despacho: RUT/email, unificar líneas por producto), [#124](https://github.com/luisurcia/mini_erp/issues/124) (feedback: **Inventario oculta productos inactivos**, **buscador de clientes** — extendido después a Insumos por producto en [#136](https://github.com/luisurcia/mini_erp/issues/136)), [#128](https://github.com/luisurcia/mini_erp/issues/128) (**color por producto**, reflejado en los gráficos del Dashboard), [#135](https://github.com/luisurcia/mini_erp/issues/135) (octava ronda: **total de unidades y buscador de cliente en Nueva Venta**, **N° de factura editable** después de creada la venta, **Top clientes ampliado** a 30 con frecuencia de compra y días desde la última compra) y [#139](https://github.com/luisurcia/mini_erp/issues/139) (novena ronda: gráfico de botellas por producto también en el modo Comparar del Dashboard, **limpieza de clientes duplicados**) — todas cerradas, más el **modo comparación del Dashboard** ([#92](https://github.com/luisurcia/mini_erp/issues/92)), el comando **`flask reset-data`** ([#116](https://github.com/luisurcia/mini_erp/issues/116)) y la **migración de las ventas reales de Scoby a producción** ([#121](https://github.com/luisurcia/mini_erp/issues/121), ver 06 Operación y 07 Pendientes). Pendientes abiertos: spike de integración bancaria (#82) y spike de notificaciones (#52).

| FECHA | ESTADO | BASE | ORIGEN |
|---|---|---|---|
| 11 septiembre 2026 | En producción | Rama `client/scoby` (commit `154d94c`) | Ingeniería inversa del código |
...
Corre en producción, en su propio ambiente (independiente del de Kombucha ERP genérico), con los **datos reales de venta de Scoby** cargados el 10 de septiembre 2026 (ver 06 Operación y 07 Pendientes).
...
- Campos: **sabor**, **nombre**, **nombre corto** (para columnas angostas), **SKU** único, **tamaño (ml)**, **precio unitario**, **color**, activo/inactivo.
...
- **Color obligatorio**: cada producto tiene un color (selector de color, con un valor por defecto si no se elige otro) — se usa para pintar de forma consistente la torta y las barras del Dashboard por producto (ver MOD-07). Se ve como una muestra de color en la lista.
...
- La matriz **muestra solo productos activos** — un producto inactivo no aparece (antes salía con una etiqueta "Inactivo"). Al desactivar un producto que todavía tiene stock, un aviso lo advierte: ese stock queda oculto en Inventario hasta reactivarlo.
...
- **Insumos por producto (receta):** en `Insumos → Insumos por producto` se define, por producto, cuántas unidades de cada insumo consume **por unidad armada**. La lista muestra solo **productos activos** (igual que Inventario, #136). Un producto sin receta no descuenta nada. Un insumo desactivado que ya está en una receta se sigue descontando. Editar recetas es **solo administrador** (es configuración de dato maestro, igual que el catálogo de productos); el bodeguero sí administra el catálogo de insumos y su stock.
...
- **Lista de Ventas:** # · fecha · cliente · N° factura (+ botón "Editar" para administrador) · estado · pago · **IVA (Sí/No)** · total, con botones **"Ver"** y **"Ticket de despacho"** por fila. Filtro por cliente (con el mismo selector con búsqueda) y por estado de pago (Todas / Por pagar / Pagado), combinables entre sí, y botón **"Descargar PDF de ventas por pagar"** — respeta el filtro de cliente activo y arma un PDF agrupado por cliente (ventas más antiguas arriba, subtotal por cliente y total a cobrar) que se comparte semanalmente con los socios para la cobranza.
...
- **Buscador** en la lista: filtra por nombre, RUT o sobrenombre (coincidencia parcial, sin distinguir mayúsculas), con la cantidad de resultados a la vista — pensado para el padrón real de cientos de clientes.
...
- **4 gráficos** en cuadrícula 2×2, todos respetan ambos filtros:
  - **Participación de ventas por producto** (torta) — reparto del monto neto por producto, coloreada con el **color de cada producto** (ver MOD-01; un producto sin color propio recibe uno de una paleta de respaldo).
  - **Botellas vendidas por producto** (barras) — unidades vendidas por producto (no monto), también con el color de cada producto, eje entero y sin leyenda.
...
- **Modo comparación** (toggle "Comparar", requiere 2+ años):
  ...
  - Los gráficos "por mes" pasan a **una serie por año**; el gráfico por producto pasa a **barras agrupadas de "Botellas vendidas por producto"** (unidades, producto × periodo, coloreado **por periodo** — igual que los otros gráficos de este modo, ver [#137](https://github.com/luisurcia/mini_erp/issues/137)); la torta se **oculta**.
...
| Dashboard | KPIs genéricos, filtro de un año | 6 KPIs del Excel de Scoby (Valor Total Pago, Total de Botellas, N° de tickets, N° de Facturas, botellas y valor unitario promedio) + filtros multi-selección de año y mes (unión, tipo tabla dinámica) + 4 gráficos 2×2 coloreados por producto (torta y barras de botellas) + modo comparación de periodos (tabla con Δ%) |
...
| Cliente | Nombre, email, teléfono, IG, notas | + RUT, sobrenombre, segmento, dirección de despacho estructurada; mínimo para crear = nombre + segmento; buscador por nombre/RUT/sobrenombre en la lista |
| Producto | Sin color | Color obligatorio, usado en los gráficos del Dashboard |
| Inventario | Muestra todos los productos (badge "Inactivo") | Solo productos activos; aviso al desactivar uno con stock |
...
- **Carga de ventas históricas (`flask import-ventas <archivo.xlsx> [--dry-run]`):** importa el Excel de ventas de Scoby directo a la base (clientes, ventas y líneas), sin tocar stock — el monto sale de la columna "total pago", el IVA se deriva por diferencia. `--dry-run` genera el mismo resumen (ventas, líneas, clientes, monto, botellas, facturas, por pagar/pagado) sin escribir nada, para revisar antes de aplicar. Deja un log en Markdown en `instance/`. Se corrió una vez en producción (ver 07 Pendientes).
- **Fusión de clientes duplicados (`flask merge-customers <descartado> <canónico> [--yes]`):** reasigna todas las ventas del cliente descartado al canónico y borra el descartado, dejando una nota con el nombre alternativo en el canónico. Pensado para los duplicados de nombre que deja la carga de ventas por planilla (mismo cliente escrito de más de una forma). Pide confirmación salvo que se pase `--yes`. Usado el 11 de septiembre 2026 para unificar 12 duplicados detectados tras la migración (ver 07 Pendientes).

## 07 Pendientes conocidos

- **Migración de datos reales — ventas e inventario cargados ([#121](https://github.com/luisurcia/mini_erp/issues/121), #119 cerrado):** el 10 de septiembre 2026 se reseteó producción (`flask reset-data`, quedando solo usuarios y configuración), se cargaron las **1.026 ventas reales** de Scoby (junio 2025 → septiembre 2026) vía `flask import-ventas` (~$47,4M, 28.732 botellas, 295 clientes en el segmento "Otros", 497 facturas, 970 pagadas / 56 por pagar) y luego el equipo cargó el **conteo real de inventario** (28 combinaciones producto×bodega, 2.972 unidades en total) — las ventas nuevas ya no están bloqueadas por falta de stock. El catálogo quedó en **10 productos** (los 8 de la planilla de ventas + "Lata Jengibre"/"Lata Maracuyá", recreados a mano por el equipo — sin color propio hasta que se editen, ver MOD-01). Falta conciliar los totales cargados contra el Excel de Scoby (#120) y revisar las ventas de la ventana de transición (9→10 de septiembre).
- **Limpieza de clientes duplicados ([#138](https://github.com/luisurcia/mini_erp/issues/138)):** la migración creó un `Customer` por cada nombre literalmente distinto en la planilla, así que la misma persona escrita de más de una forma (ej. "Niklitschek" / "Niklitshek") quedó duplicada. El 11 de septiembre 2026 se revisaron los 295 clientes cargados y se fusionaron **12 duplicados confirmados** (ventas reasignadas al cliente canónico, sin perder historial) — el padrón quedó en **283 clientes**. Herramienta nueva y reutilizable: `flask merge-customers <descartado> <canónico>`.
- **Insumos: catálogo cargado, stock y recetas pendientes.** El equipo creó el catálogo de Insumos (9 ítems: botella, una etiqueta por sabor, 2 latas) pero **su stock sigue en 0** y **no hay recetas cargadas en `Insumos → Insumos por producto`** — sin eso, el consumo automático al armar (ver MOD-03) no descuenta nada todavía. Falta: reponer el stock inicial de insumos y, para cada producto, cuántas unidades de cada insumo lleva.
- **Bodega de Fermentación en producción:** la que el equipo había creado a mano ("En Fermentación") se renombró a **"Bodega de Fermentación"** y quedó con su rol de flujo; el conteo cargado tras la migración ya la incluye.
...
*Scoby ERP — PRD por ingeniería inversa · rama `client/scoby` · commit `154d94c` · 11 septiembre 2026*

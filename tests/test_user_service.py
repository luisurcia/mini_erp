...
def test_create_user_sets_role_and_hashes_password(app):
    service = UserService()
    user = service.create_user("bodeguero1", "password123", User.ROLE_BODEGUERO)
    assert user.role == User.ROLE_BODEGUERO
...
def test_create_user_duplicate_username_raises(app):
    ...
    with pytest.raises(DuplicateUsernameError):
        service.create_user("dupe", "otherpass", User.ROLE_BODEGUERO)


def test_update_user_changes_username_and_role(app):
    service = UserService()
    user = service.create_user("original", "password123", User.ROLE_VENTAS)
    service.update_user(user, username="renamed", role=User.ROLE_BODEGUERO)
...
def test_update_user_password_only_changes_when_provided(app):
    service = UserService()
    user = service.create_user("pwtest", "password123", User.ROLE_VENTAS)
    service.update_user(user, username="pwtest", role=User.ROLE_VENTAS, password=None)
    assert user.check_password("password123")

    service.update_user(user, username="pwtest", role=User.ROLE_VENTAS, password="newpass456")
...
def test_delete_user_removes_it(app):
    ...
    victim = service.create_user("victim", "password123", User.ROLE_VENTAS)
...
def test_cannot_demote_last_admin(app):
    ...
    with pytest.raises(LastAdminError):
        service.update_user(admin, username="sole_admin", role=User.ROLE_BODEGUERO)
...
def test_can_demote_admin_when_another_admin_remains(app):
    ...
    service.update_user(admin1, username="admin1", role=User.ROLE_BODEGUERO)
    assert admin1.role == User.ROLE_BODEGUERO
